...
export const pageActions = (page: Page, isMobile?: boolean) => {
    ...
    const openItemnav = async () => {
        await (isMobile
            ? main.tap({ position: MOBILE_ITEMNAV_OPEN_BUTTON })
            : main.click({ position: DESKTOP_ITEMNAV_OPEN_BUTTON }));
    };
    const closeItemnav = async () => {
        await (isMobile
            ? main.tap({ position: MOBILE_ITEMNAV_CLOSE_BUTTON })
            : main.click({ position: DESKTOP_ITEMNAV_CLOSE_BUTTON }));
    };
