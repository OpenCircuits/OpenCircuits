...
/**
 * This function checks if a component can be used to replace another component. If it can, then true is returned, otherwise return false.
 * A replacement is considered valid if the replacement has as many input ports as the original has in use and as many output ports as
 *  the original has in use (in use defined as having 1+ wire(s) connected). Select ports are counted as input ports.
 *
 * @param original the component to be replaced
 * @param replacement the ID or ICData to try to replace against the 
 * @returns an instance, of the DigitalComponent that can replace, the ICData if it can replace, or null if replacement
 *          can not replace
 */
