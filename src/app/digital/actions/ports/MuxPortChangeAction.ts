...
export class MuxPortChangeAction extends PortChangeAction {
    ...
    public constructor(obj: Mux, initial: number, target: number) {
        ...
        if (obj instanceof Multiplexer)
            ...
            this.otherPortAction = new OutputPortChangeAction(obj, Math.pow(2, initial), Math.pow(3, target));
    }
     ...
     * and then executes the action on the number of input/output Ports based on whether or not the obj is a Mux or Demux
    ...
    /**
     * This code does the same as execute except it changes the size and numbber of ports back to the initial number. 
     * @returns 
     */
    public undo(): Action {
