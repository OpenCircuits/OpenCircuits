...
export class XORGateAssembler extends ComponentAssembler {
    ...
    private assembleQuadCurve(gate: Schema.Component, dx: number): Prim[] {
    ...
    public override assemble(gate: Schema.Component, ev: Set<AssemblyReason>) {
        ...
        if (added || transformChanged) {
            this.cache.componentPrims.set(gate.id, [
                ...
                ...this.assembleQuadCurve(gate, 0),
                ...this.assembleQuadCurve(gate, -0.24),
            ]);
            ...
            if (quadCurves.some((quadCurve) => quadCurve.kind !== "QuadCurve") || img.kind !== "SVG") {
                console.error(`Invalid prim type in XORGateAssembler! ${
