...
export class CircuitInternal {
  ...
  private applyOp(op: CircuitOp): Result {
    ...
    const checkWireConnectivity = (
      ...
      this.componentPortsMap.get(p1.parent)?.forEach((id) => {
        ...
        map.set(
          p1,
          [...to].map((id) => this.getPortByID(id).unwrap())
        );
      });
      return this.getComponentAndInfoByID(p1.parent).andThen(([_, info]) =>
        info
          .checkPortConnectivity(map)
          .mapErr(
            AddErrE(
              `Adding wire from port ${p1} to ${p2} is creates an illegal configuration.`
            )
          )
      );
    };
    switch (op.kind) {
      case "PlaceComponentOp": {
        if (op.inverted) {
          if (!this.componentPortsMap.has(op.c.id))
            throw new Error(
              `Deleted component ${op.c.id} should have componentPortsMap initialized!`
            );
          if (this.componentPortsMap.get(op.c.id)!.size > 0)
            throw new Error(
              `Deleted component ${op.c.id} should not have ports`
            );
          ...
          if (this.componentPortsMap.has(op.c.id))
            throw new Error(
              `Placed component ${op.c.id} should not have any ports`
            );
      ...
      case "SetComponentPortsOp": {
        const [newPorts, oldPorts] = op.inverted
          ? [op.oldPorts, op.newPorts]
          : [op.newPorts, op.oldPorts];

        if (newPorts) {
          const res = this.getComponentAndInfoByID(op.component).andThen(
            ([_, info]) =>
              CheckPortList(info, newPorts).mapErr(
                AddErrE(
                  `Invalid new port list ${newPorts} for component ${op.component}`
                )
              )
          );
        ...
        if (!op.inverted) op.deadWires?.forEach(deleteWire);
        oldPorts?.forEach(deletePort);
        newPorts?.forEach(addPort);
        if (op.inverted) op.deadWires?.forEach(addWire);
        return OkVoid();
      }
      case "ConnectWireOp": {
        return this.getPortByID(op.w.p1).andThen((p1) =>
          this.getPortByID(op.w.p2)
            .andThen((p2) =>
              checkWireConnectivity(p1, p2).and(checkWireConnectivity(p2, p1))
            )
            .map((_) => {
      ...
      case "SetPropertyOp": {
        return this.getObjectAndInfoByID(op.id).andThen(([obj, info]) =>
          info.checkPropValue(op.key, op.newVal).uponOk(() => {
            // Copy-on-write
            obj.props = { ...obj.props };
            if (op.newVal) obj.props[op.key] = op.newVal;
  ...
  private getPortPortMapChecked(id: GUID): Set<GUID> {
    ...
    if (!p)
      throw new Error(
        `Invariant Violation: getPortPortMapChecked(${id}) unexpectedly returned undefined`
      );
    return p;
  }
  ...
  public constructor(objInfo: ObjInfoProvider, log: CircuitLog) {
    ...
    this.log.subscribe((evt) => {
      ...
      if (this.transaction) this.transformTransaction(evt.ops);
  ...
  private applyOpsChecked(ops: CircuitOp[]): void {
    ...
    ResultUtil.mapIter(ops.values(), this.applyOp)
      .mapErr(
        AddErrE("Internal applyOp failed!  Client state is inconsistent!")
      )
      .unwrap();
  }
  ...
  public commitTransaction(): void {
    this.endTransactionHelper((txOps) => {
      ...
      if (this.clock !== this.log.clock)
        throw new Error(
          `Unexpected clock difference (${this.clock} vs ${this.log.clock})` +
  ...
  public deleteComponent(id: GUID): Result {
    return this.getCompByID(id).andThen((c) =>
  ...
  public deleteWire(id: GUID): Result {
    return this.getWireByID(id).andThen((w) =>
      this.addTransactionOp({
        kind: "ConnectWireOp",
        inverted: true,
        w, // No copy needed
  ...
  public getObjectInfo(kind: string): ObjInfo {
    ...
    if (!info) throw new Error(`Unknown obj type ${kind}!`);
    return info;
  }
  public getComponentInfo(kind: string): ComponentInfo {
    const info = this.objInfo.getComponent(kind);
    if (!info) throw new Error(`Unknown component type ${kind}!`);
    return info;
  }

  private getObjectAndInfoByID(id: GUID): Result<[Schema.Obj, ObjInfo]> {
    return this.getMutableObjByID(id).map((obj) => [
      obj,
      this.getObjectInfo(obj.kind),
    ]);
  }

  private getComponentAndInfoByID(
    id: GUID
  ): Result<[Schema.Component, ComponentInfo]> {
    return this.getCompByID(id).map((c) => [c, this.getComponentInfo(c.kind)]);
  }
  ...
  public setPortConfig(id: GUID, portConfig: PortConfig): Result {
    ...
    return this.getComponentAndInfoByID(id)
      ...
      .andThen((newPorts) => {
        ...
        const oldPorts = [...this.getPortsForComponent(id).unwrap()].map((id) =>
          this.getPortByID(id).unwrap()
        );
  ...
  private getBaseKindByID<O extends Schema.Obj>(
    id: GUID,
    kind: O["baseKind"]
  ): Result<O> {
    return this.getObjByID(id).andThen((obj): Result<O> => {
      if (obj.baseKind !== kind)
        return ErrE(
          `CircuitInternal: Attempted to get ${kind} by ID ${id} but received ${obj.baseKind}!`
        );
      return Ok(obj as O);
    });
  }
