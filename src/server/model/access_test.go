...
func TestUserNoAccess(t *testing.T) {
	for i := int32(1); i <= AccessCreator; i++ {
		for j := int32(1); j <= AccessCreator; j++ {
	...
	for i := int32(0); i <= AccessCreator; i++ {
		for j := int32(0); j <= AccessCreator; j++ {
			if userPerm(i).CanUpdateUser(creater(), userPerm(j)) {
				t.Error("Creator user could be demoted")
			}
		}
	}
	for i := int32(0); i < AccessCreator; i++ {
		if creater().CanUpdateUser(creater(), userPerm(i)) {
			t.Error("Creator user could demote self")
		}
	}
}
