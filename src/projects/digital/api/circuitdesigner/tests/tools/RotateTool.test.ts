...
describe("RotateTool", () => {
    describe("Single Object", () => {
        test("Rotate Component 45° CCW from side", () => {
            ...
            input.move(V(-ROTATION_CIRCLE_RADIUS, 0)).press().move(V(0, +ROTATION_CIRCLE_RADIUS)).release();
        ...
        test("Rotate Component 45° CW from top", () => {
            ...
            input.move(V(0, +ROTATION_CIRCLE_RADIUS)).press().move(V(+ROTATION_CIRCLE_RADIUS, 0)).release();
        ...
        test("Rotate Object 100 times around", () => {
            ...
            input.click(V(0, 0)); // Select object
            input.move(V(ROTATION_CIRCLE_RADIUS, 0)).press();
    ...
    describe("Multiple Objects", () => {
        test("Rotate Objects 45° CW", () => {
            ...
            input.drag(V(-4, -4), V(+4, +4)); // Select objects
            expect(circuit.selections).toHaveLength(2);

            const obj1Pos = obj1.pos,
                obj2Pos = obj2.pos;
        ...
        test("Rotate Objects 100 times around", () => {
            const [{ circuit }, input, _, { PlaceAt }] = CreateCircuitDesigner();
            const [obj1, obj2, obj3] = PlaceAt(["Switch", V(-2, -2)], ["LED", V(2, 0)], ["ANDGate", V(0, 2)]);
            ...
            const obj1Pos = obj1.pos,
                ...
                obj3Pos = obj3.pos;
            const midpoint = circuit.selections.midpoint;
            input.moveTo(midpoint).move(V(ROTATION_CIRCLE_RADIUS, 0)).press();
            ...
            expect(obj3.angle).toBeCloseTo(0);
        ...
        test("Rotate Objects 45° CW then undo", () => {
            ...
            input.pressKey("Control").pressKey("z").releaseKey("Control").releaseKey("z");
        ...
        test("Rotate Objects 45° CW while pressing Z then undo/redo", () => {
            ...
            // redo
            input.pressKey("Control").pressKey("y").releaseKey("Control").releaseKey("y");
        ...
        test("Rotate Objects 180° CW while toggling Z halfway through", () => {
            ...
            input
                .releaseKey("z") // release z partway through
                .move(V(ROTATION_CIRCLE_RADIUS, -ROTATION_CIRCLE_RADIUS))
                .release();
