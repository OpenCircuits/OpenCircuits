...
type LocalPropagatorFunc = (
    obj: Schema.Component,
...
type StateProps = Record<
    string,
    (c: Schema.Component, curState?: number[], tickInfo?: { curTick: number }) => number[]
>;
...
function MakeLocalPropagator(func: LocalPropagatorFunc, stateProps?: StateProps): PropagatorInfo {
    return {
        propagator: (comp, info, state, tickInfo) => {
            ...
            const inputSignals = Object.fromEntries(
                info.inputPortGroups.map((group) => [
                    group,
                    ports[group].map((id) => state.signals.get(id) ?? Signal.Off),
                ]),
            );
            ...
            return {
                outputs: new Map(
                    Object.entries(outputs).flatMap(([group, signals]) =>
        ...
        getNewStateForPropChange: stateProps
            ? (prop, c, curState, tickInfo) => stateProps[prop](c, curState, tickInfo)
            : undefined,
    };
}
...
function MakeSingleOutputPropagator(
    getOutput: (
        ...
        signals: Record<string, Signal[]>,
        state?: number[],
        tickInfo?: { curTick: number; lastStateTick?: number },
    ) => {
        signal: Signal;
        nextState?: number[];
        nextCycle?: number;
...
const [ANDGate, NANDGate] = MakeGatePropagators((inputs) =>
    inputs.some(Signal.isOff) ? Signal.Off : inputs.some(Signal.isMetastable) ? Signal.Metastable : Signal.On,
);
const [ORGate, NORGate] = MakeGatePropagators((inputs) =>
    inputs.some(Signal.isOn) ? Signal.On : inputs.some(Signal.isMetastable) ? Signal.Metastable : Signal.Off,
);
...
function MakeFlipFlopPropagator(getState: (signals: Record<string, Signal[]>, state: Signal, up: boolean) => Signal) {
    return MakeLocalPropagator((_obj, signals, state = [Signal.Off, Signal.Off]) => {
        const [curState, prevClk] = state,
            [CLK] = signals["clk"],
            [PRE] = signals["pre"],
            [CLR] = signals["clr"];
...
function MakeTimedPropagator(onTick: LocalPropagatorFunc, defaultDelay: number, defaultExtraState: number[] = []) {
    return MakeLocalPropagator(
        (obj, signals, [lastStateTick, pausedTick, ...rest] = [-1, -1, ...defaultExtraState], tickInfo) => {
        ...
        },
        {
            ...
            "paused": (c, [lastStateTick, pausedTick, ...rest] = [-1, -1, ...defaultExtraState], tickInfo) => [
                lastStateTick,
                c.props["paused"] === true ? tickInfo!.curTick : pausedTick,
                ...rest,
            ],
        },
    );
...
export const DigitalPropagators: PropagatorsMap = {
    ...
    "ConstantNumber": MakeLocalPropagator(
        (obj, _signals, _state) => {
    ...
    "Clock": MakeTimedPropagator(
        (obj, _signals, [curSignal] = [Signal.Off], _tickInfo) => ({
    ...
    "Oscilloscope": MakeTimedPropagator((obj, signals, state = [], _tickInfo) => {
        ...
        const nextSignals = new Array<Signal>(8).fill(Signal.Off).map((_, i) => signals["inputs"][i]);
        return {
            outputs: {},
    ...
    // Other
    "Multiplexer": MakeStatelessSingleOutputPropagator(
        (_obj, signals, _state) =>
