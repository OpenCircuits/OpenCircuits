    ...
    public constructor(
        ...
        sim.subscribe((ev) => {
            const [comps, inputPorts, outputPorts] = (
                ...
                .map((paths) => [...paths].filter((p) => p.length === 1).map((p) => p[0]));

            comps.forEach((compId) => this.dirtyComponents.add(compId, AssemblyReason.StateUpdated));
            outputPorts.forEach((portId) => {
                const wires = circuit.getWiresForPort(portId).unwrap();
                wires.forEach((wireId) => this.dirtyWires.add(wireId, AssemblyReason.SignalsChanged));
            });
