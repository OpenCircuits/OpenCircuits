...
export class OscilloscopeAssembler extends ComponentAssembler {
            ...
            [
                ...
                },
                {
                    ...
                    assemble: (comp) => {
                        ...
                        return {
                            kind: "Group",
                            prims: new Array(this.numPorts(comp))
                                .fill(0)
                                .map((_, i) => this.assembleDisplay(comp, allSignals, i)),
                            ignoreHit: true,
                        };
                    },
    ...
    protected assembleDisplay(comp: Schema.Component, allSignals: Signal[][], i: number) {
        ...
        const positions = allSignals.map((s, j) =>
            V(
        ...
        return {
            ...
            points: [
                positions[0],
                ...positions.flatMap((pos, j) => {
                    // Draws a vertical line so that the jump looks better
                    //  from 0 -> 1 or 1 -> 0
                    if (j > 0 && allSignals[j - 1][i] !== allSignals[j][i]) return [pos, pos.add(dx, 0)];
                    return pos.add(dx, 0);
                }),
    ...
    private getDisplaySize(comp: Schema.Component) {
        ...
        return V((c.props["w"] as number) ?? 8, (c.props["h"] as number) ?? 4);
    }
}
