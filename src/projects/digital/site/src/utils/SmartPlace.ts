...
export function SmartPlace(
    ...
    for (let i = 0; i < N; i++) {
        ...
        const inputs = inputPorts
            .toSorted(
                ({ targetPos: targetPosA }, { targetPos: targetPosB }) =>
        ...
        inputs.forEach((s, i) => {
            s.pos = V(
                -compBoundsWithPorts.size.x / 2 - AUTO_PLACE_SWITCH_SPACE,
                ((inputPorts.length - 1) / 2 - i) * s.bounds.size.y,
            ).add(comp.pos);
        });
        outputs.forEach((l, i) => {
            l.pos = V(
                compBoundsWithPorts.size.x / 2 + AUTO_PLACE_LED_SPACE * (i + 1),
                // This centers the LED around the port of the LED
                outputPorts[i].targetPos.y - comp.y - (l.inputs[0].targetPos.y - l.y),
            ).add(comp.pos);
        });
