...
function ConvertCompProps(comp: Entry<V3_0Schema.DigitalComponent>): ProtoSchema.Component["otherProps"] {
...
function FindPorts(comp: Entry<V3_0Schema.DigitalComponent>): {
    cfgIdx: number;
    ports: Array<[ProtoSchema.Port, Entry<V3_0Schema.DigitalPort>]>;
} {
    const inputs = comp.inputs.currentPorts,
        outputs = comp.outputs.currentPorts;
    switch (comp.type) {
...
function ConvertComponent(
    ...
    ProtoSchema.Component,
    Entry<V3_0Schema.DigitalComponent>,
    Array<[ProtoSchema.Port, Entry<V3_0Schema.DigitalPort>]>,
] {
    ...
    return [
        {
            ...
            color:
                "color" in wire && wire["color"] && wire["color"] !== "#ffffff"
                    ? parseInt((wire["color"] as string).slice(1), 16)
                    : undefined,
...
function ConvertIC(
    ...
    const { componentsAndPorts, wires } = ConvertObjects(
        ic.collection.components,
        ic.collection.wires,
        refToICIDMap,
        allICIDs,
    );
    ...
    const inputPins = ic.collection.inputs
        .map(
            (obj, i) =>
                [componentsAndPorts.findIndex(([_, parent]) => parent.ref === obj.ref), ic.inputPorts[i]] as const,
        )
        .map(([internalCompIdx, port]) => ConvertICPin(port, internalCompIdx, 0, ic.transform.size));
    const outputPins = ic.collection.outputs
        .map(
            (obj, i) =>
                [componentsAndPorts.findIndex(([_, parent]) => parent.ref === obj.ref), ic.outputPorts[i]] as const,
        )
    ...
    const icStates = componentsAndPorts
        ...
        .map(([_comp, entry]) => {
            ...
            const componentsAndPorts = icDataObjs.componentsAndPorts
                ...
                .zip(icObjs.componentsAndPorts.sort(([_c1, e1], [_c2, e2]) => compareComps(e1, e2)))
                .sort(([[i1]], [[i2]]) => i1 - i2)
                .map(
                    ([[_i, comp1, _e1, _ports1], [_comp2, e2, ports2]]) =>
                        [comp1, e2, ports2] satisfies [
                            ProtoSchema.Component,
                            Entry<V3_0Schema.DigitalComponent>,
                            Array<[ProtoSchema.Port, Entry<V3_0Schema.DigitalPort>]>,
                        ],
...
export function V3_0Migrator(circuit: V3_0Schema.Circuit) {
    ...
    const allICEntriesMap: Record<string, Entry<V3_0Schema.ICData>> = Object.fromEntries(
    ...
    const ics = [...new Set(Object.values(refToICsMap))]
        ...
        .sort(
            ([_i1, e1], [_i2, e2]) =>
                uniqueICIDs.indexOf(refToICIDMap[e1.ref]) - uniqueICIDs.indexOf(refToICIDMap[e2.ref]),
        );

    const { componentsAndPorts, wires } = ConvertObjects(
        root.designer.objects,
        root.designer.wires,
        refToICIDMap,
        uniqueICIDs,
    );
