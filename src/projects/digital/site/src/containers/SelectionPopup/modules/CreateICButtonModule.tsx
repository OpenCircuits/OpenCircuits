...
export const CreateICButtonModule = ({ circuit }: Props) => {
    ...
    return (
        <button type="button" title="Create an IC from selections" onBlur={() => setShowError(false)} onClick={open}>
            Create IC
            {icValidationStatus !== ICValidationStatus.Valid && showError && (
                <span className="tooltip">{errorMessages[icValidationStatus]}</span>
            )}
        </button>
    );
};
