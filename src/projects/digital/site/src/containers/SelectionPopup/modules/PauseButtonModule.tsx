...
export const PauseButtonModule = ({ circuit }: Props) => {
    ...
    const onClick = () => {
        cs.forEach((c) => c.setProp("paused", !isPaused));
    };

    if (isPaused) {
        return (
            <button type="button" title="Resume the selected components" onClick={onClick}>
                Resume
            </button>
        );
    }
    ...
    return (
        <button type="button" title="Pause the selected components" onClick={onClick}>
            Pause
        </button>
    );
};
