...
import { Generate, InputTypes, OutputTypes } from "./generate";
...
export const ExprToCircuitPopup = ({ circuit, viewport: { camera } }: Props) => {
    ...
    return (
        <Popup
            title="Digital Expression To Circuit Generator"
        ...
        >
            <div className="exprtocircuit__popup">
                ...
                <InputField
                    title="Enter Circuit Expression"
                    type="text"
                    value={expression}
                ...
                <div className="exprtocircuit__popup__settings">
                    <div>
                        ...
                        <ButtonToggle isOn={format === "custom"} height="40px" onChange={() => setFormat("custom")}>
                            Custom
                        </ButtonToggle>
                        {format === "custom" && <CustomOps customOps={customOps} setCustomOps={setCustomOps} />}
                    </div>

                    <div>
                        <h3>Options</h3>
                        <BooleanOption option={label} setOption={setLabel} text="Place labels for inputs" isDisplayed />
                        <BooleanOption option={isIC} setOption={setIsIC} text="Generate into IC" isDisplayed />
                        <BooleanOption
                            isDisplayed={output === "Oscilloscope" && input === "Clock"}
