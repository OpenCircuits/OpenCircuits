        ...
        <div>
            {operationReps.map((op) => (
                <div key={op[0]} className="exprtocircuit__popup__customOps">
                    <InputField
                        type="text"
                        title={"Enter symbol for " + op[0]}
                        value={customOps.ops[op[1]]}
                        onChange={(e) =>
                            setCustomOps({ ...customOps, ops: { ...customOps.ops, [op[1]]: e.target.value } })
                        }
                    />
                    <label htmlFor={customOps.ops[op[1]]}>
                        Custom {op[0]}: &quot;{customOps.ops[op[1]]}&quot;
                    </label>
                </div>
            ))}
            <div className="exprtocircuit__popup__customOps">
                <InputField
                    title="Enter symbol for Separator"
                    type="text"
                    value={customOps.separator}
