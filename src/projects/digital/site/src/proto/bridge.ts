...
export function DigitalCircuitToProto(circuit: DigitalCircuit): DigitalProtoSchema.DigitalCircuit {
    function ConvertSignal(signal: Signal): number {
        return signal === Signal.Off ? 0 : signal === Signal.On ? 1 : signal === Signal.Metastable ? 2 : 0;
    }

    function ConvertSimState(
        circuit: ReadonlyDigitalCircuit,
        state: DigitalSchema.DigitalSimState,
        icId?: Schema.GUID,
    ): DigitalProtoSchema.DigitalSimState {
        // MUST SORT BY THE SAME METRIC AS `CircuitToProto`
        const comps = (icId ? circuit.getIC(icId)!.components : circuit.getComponents()).sort(
            (c1, c2) => c1.zIndex - c2.zIndex,
        );
        ...
        return {
            ...
            states: comps.filter((c) => StateFullKinds.has(c.kind)).map((c) => ({ state: state.states[c.id] })),

            icStates: comps.filter((c) => c.isIC()).map((c) => ConvertSimState(circuit, state.icStates[c.id], c.kind)),
        };
    }
...
export function DigitalProtoToCircuit(proto: DigitalProtoSchema.DigitalCircuit): DigitalCircuit {
    function ConvertSignal(signal: number): Signal {
        return signal === 0 ? Signal.Off : signal === 1 ? Signal.On : signal === 2 ? Signal.Metastable : Signal.Off;
    }

    function ConvertSimState(
        circuit: ReadonlyDigitalCircuit,
        state: DigitalProtoSchema.DigitalSimState,
        icId?: Schema.GUID,
    ...
    if (!proto.circuit) throw new Error(`DigitalProtoToSchema: Failed to find circuit! ${proto}`);
    if (!proto.simState) throw new Error(`DigitalProtoToSchema: Failed to find simState! ${proto}`);
