...
export function PrintDebugStats(proto: DigitalProtoSchema.DigitalCircuit) {
    const CalcStats = (b: Uint8Array[]) => {
        ...
        return {
            ...
            max: Math.max(...lens),
            distrib: lens.reduce(
                (counts, cur) => ({ ...counts, [cur]: (counts[cur] ?? 0) + 1 }),
    ...
    console.debug(
        `    Metadata: ${ProtoSchema.CircuitMetadata.encode(proto.circuit!.metadata!).finish().byteLength} bytes`,
    );
    ...
    encodedICs.zip(ics).forEach(([b, ic], i) => {
        ...
        console.debug(
            `        Metadata: ${ProtoSchema.IntegratedCircuitMetadata.encode(ic.metadata!).finish().byteLength} bytes`,
        );
        ...
        console.debug(
            `          Avg: ${compStats.avg} bytes / comp, Min: ${compStats.min} bytes, Max: ${compStats.max} bytes`,
        );
        console.debug(
            `          Distribution: ${Object.keys(compStats.distrib)
                .map((s) => parseInt(s))
                .sort((a, b) => a - b)
                .map((key) => key + ":" + compStats.distrib[key])
                .join(", ")}`,
        );
        ...
        console.debug(
            `          Avg: ${wireStats.avg} bytes / wire, Min: ${wireStats.min} bytes, Max: ${wireStats.max} bytes`,
        );
        console.debug(
            `          Distribution: ${Object.keys(wireStats.distrib)
                .map((s) => parseInt(s))
                .sort((a, b) => a - b)
                .map((key) => key + ":" + wireStats.distrib[key])
                .join(", ")}`,
        );
    });
    ...
    const encodedIcSimStates = proto.icInitialSimStates!.map((ic) =>
        DigitalProtoSchema.DigitalSimState.encode(ic).finish(),
    );
    const icSimStateStats = CalcStats(encodedIcSimStates);
    console.debug(`  ICInitialSimStates: ${icSimStateStats.sum} bytes`);
    encodedIcSimStates.forEach((c, i) => console.debug(`    ${i}: ${c.byteLength} bytes`));
    console.debug(
        `  SimState: ${DigitalProtoSchema.DigitalSimState.encode(proto.simState!).finish().byteLength} bytes`,
    );
}
