...
export class CircuitImpl<T extends CircuitAPITypes> extends ObservableImpl<CircuitEvent> implements Circuit {
    ...
    public getComponents(): T["Component[]"] {
        return [...this.internal.getComps()].map((id) => this.ctx.factory.constructComponent(id));
    }
    public getWires(): T["Wire[]"] {
        return [...this.internal.getWires()].map((id) => this.ctx.factory.constructWire(id));
    }
    ...
    public createContainer(objs: GUID[]): T["ObjContainerT"] {
        ...
        for (const id of objSet) {
            if (!this.internal.hasComp(id) && !this.internal.hasWire(id) && !this.internal.hasPort(id)) {
    ...
    public deleteObjs(objs: Array<T["Component"] | T["Wire"]>): void {
        ...
        const wirePaths = objs
            .filter((o): o is T["Wire"] | T["Node"] => o.baseKind === "Wire" || o.isNode())
            .flatMap((o) => o.path);
        const wireIds = new Set(wirePaths.filter((o): o is T["Wire"] => o.baseKind === "Wire").map((o) => o.id));
    ...
    public getICs(): T["IC[]"] {
        return [...this.internal.getICs()].map((id) => this.ctx.factory.constructIC(id));
    }
    ...
    protected doImport(
        circuit: T["ReadonlyCircuit"] | T["ReadonlyObjContainerT"],
        opts?: { refreshIds?: boolean; loadMetadata?: boolean },
    ): Map<GUID, GUID> {
        const refreshIds = opts?.refreshIds ?? false,
            loadMetadata = opts?.loadMetadata ?? false;

        const isCircuit = (o: T["ReadonlyCircuit"] | T["ReadonlyObjContainerT"]): o is T["ReadonlyCircuit"] =>
            o instanceof CircuitImpl;
