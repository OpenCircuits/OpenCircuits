...
export class LabelAssembler extends ComponentAssembler {
    ...
    private getTextColor(comp: Schema.Component) {
        return (
            (this.circuit.getCompByID(comp.id).unwrap().props["textColor"] as string | undefined) ??
            this.options.defaultFontColor
        );
    }

    private getBGColor(comp: Schema.Component) {
        return (
            (this.circuit.getCompByID(comp.id).unwrap().props["bgColor"] as string | undefined) ??
