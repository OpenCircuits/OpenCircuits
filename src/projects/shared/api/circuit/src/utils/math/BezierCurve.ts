...
export class BezierCurve extends Curve {
    ...
    public override getX(t: number): number {
        const it = 1 - t;
        return (
            this.p1.x * it * it * it + 3 * this.c1.x * t * it * it + 3 * this.c2.x * t * t * it + this.p2.x * t * t * t
        );
    }
    public override getY(t: number): number {
        const it = 1 - t;
        return (
            this.p1.y * it * it * it + 3 * this.c1.y * t * it * it + 3 * this.c2.y * t * t * it + this.p2.y * t * t * t
        );
    }
