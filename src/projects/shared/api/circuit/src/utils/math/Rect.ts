...
export class Rect {
    ...
    public contains(pt: Vector | Rect): boolean {
        if (pt instanceof Rect) {
            return pt.right >= this.left && pt.left <= this.right && pt.top <= this.top && pt.bottom >= this.bottom;
        }
        return pt.x <= this.right && pt.x >= this.left && pt.y <= this.top && pt.y >= this.bottom;
    }
    ...
    public shift(
        dir: Vector,
        amt: Vector,
        constraints?: { minSize?: { width: number; height: number }; bounds?: Rect },
    ): Rect {
        ...
        return Rect.From({
            left:
                dir.x < 0 // Shift left
                    ? Clamp(this.left + shift.x, bounds.left, this.right - minSize.width)
                    : this.left,

            right:
                dir.x > 0 // Shift right
                    ? Clamp(this.right + shift.x, this.left + minSize.width, bounds.right)
                    : this.right,

            bottom:
                dir.y < 0 // Shift down
                    ? Clamp(this.bottom + shift.y, bounds.bottom, this.top - minSize.height)
                    : this.bottom,

            top:
                dir.y > 0 // Shift up
                    ? Clamp(this.top + shift.y, this.bottom + minSize.height, bounds.top)
                    : this.top,
        });
    }
    ...
    public static From(bounds: RectProps, yIsUp = true): Rect {
        ...
        return new Rect(
            V(GetCenter(boundsX), GetCenter(boundsY) * (yIsUp ? +1 : -1)),
