...
export class PanTool extends ObservableImpl<ToolEvent> implements Tool {
        ...
        return (
            ...
            (ev.type === "keydown" &&
                (ev.key === "Alt" ||
                    ...
                    ev.key === "ArrowUp" ||
                    ev.key === "ArrowDown")) ||
            (ev.type === "mousedrag" && (ev.button === MIDDLE_MOUSE_BUTTON || ev.input.touchCount === 2))
        );
    }
    ...
    public onEvent(ev: InputAdapterEvent, { viewport }: CircuitDesigner): void {
        ...
        if (ev.type === "keydown") {
            ...
            const dx = (ev.input.keysDown.has("ArrowLeft") ? -1 : 0) + (ev.input.keysDown.has("ArrowRight") ? +1 : 0);
            const dy = (ev.input.keysDown.has("ArrowUp") ? +1 : 0) + (ev.input.keysDown.has("ArrowDown") ? -1 : 0);
