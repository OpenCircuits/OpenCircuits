...
export const WiringToolRenderer = (getColor: (params: ToolRendererArgs) => string | undefined): ToolRenderer => ({
    render: (args) => {
        const {
            designer: { curTool, circuit, viewport },
            renderer,
        } = args;

        // If a non-selection-box-tool active, then do nothing
        if (!(curTool instanceof WiringTool)) return;
        ...
        renderer.draw({
            ...
            style: {
                ...
                stroke: {
                    ...
                    color: targetCanConnectPort
                        ? renderer.options.selectedFillColor
        ...
        if (targetCanConnectPort) {
            const POS_OFFSET = V(-1, 1),
                STROKE_WIDTH = 3;

            // Draw a darker check underneath to make the bright green be a bit more visible
            DrawCheck(renderer, pos2.add(POS_OFFSET), LEN * 0.8, 1 / 2, "#000000", STROKE_WIDTH);
            DrawCheck(
                renderer,
                pos2.add(POS_OFFSET),
                LEN * 0.8,
            ...
            DrawX(renderer, pos2, LEN, renderer.options.defaultInvalidColor, STROKE_WIDTH);
        }
    },
});
