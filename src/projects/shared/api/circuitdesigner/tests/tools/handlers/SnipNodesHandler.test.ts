...
describe("SnipNodesHandler", () => {
    test("Simple snip", () => {
        ...
        input.moveTo(V(1, 0)).press().moveTo(V(6, 0)).release();

        expect(obj1).toBeConnectedTo(obj2);

        input.moveTo(V(3, 0)).press().move(V(0, 2)).release();
    ...
    test("Multiple snips", () => {
        ...
        input.moveTo(V(1, 0)).press().moveTo(V(11, 0)).release();

        expect(obj1).toBeConnectedTo(obj2);

        input.moveTo(V(2, 0)).press().move(V(0, 1)).release();

        const node1 = selections.components[0];

        input.moveTo(V(9, 0.2)).press().move(V(0, 1)).release();

        const node2 = selections.components[0];

        expect(node1).not.toBeObj(node2);

        input.moveTo(V(5, 1)).press().move(V(0, 1)).release();
