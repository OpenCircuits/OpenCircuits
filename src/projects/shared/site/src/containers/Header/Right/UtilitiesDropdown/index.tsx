...
export const UtilitiesDropdown = ({ extraUtilities }: Props) => {
    ...
    const { curMenu, isLocked, isSaved } = useSharedSelector((state) => ({
    ...
    useEffect(() => {
        if (process.env.NODE_ENV === "development") {
            DevListFiles().then((result) => setEnableReload(result.includes(DEV_CACHED_CIRCUIT_FILE)));
        }
    }, [setEnableReload]);
                ...
                <>
                    ...
                    <div
                        ...
                        onClick={async () => {
                            dispatch(CloseHeaderMenus());
                            await DevCreateFile(
                                CircuitHelpers.SerializeAsString(designer.circuit),
                                DEV_CACHED_CIRCUIT_FILE,
                            );
                    ...
                    {enableReload && (
                        <div role="button" tabIndex={0} onClick={load}>
