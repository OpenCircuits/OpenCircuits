...
export const VectorModuleInputField = ({
    ...
    const [state, setState] = useBaseModule<[number, number]>({
        ...
        getCustomDisplayVal: ([x, y], i) => {
            ...
            const display = (getCustomDisplayVal ?? ((v) => V(parseFloat(v.x.toFixed(2)), parseFloat(v.y.toFixed(2)))))(
                V(x, y),
            );
        ...
        <>
            <NumberInputField
                ...
                onBlur={(e) => {
                    ...
                    if (e.relatedTarget !== yRef.current) setState.onBlur();
                }}
            />
            <NumberInputField
                ref={yRef}
                ...
                onBlur={(e) => {
                    // Allows the action to only be submitted once both fields have blurred
                    if (e.relatedTarget !== xRef.current) setState.onBlur();
                }}
            />
        </>
    );
