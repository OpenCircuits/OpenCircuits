...
export const useSelectionProps = <O extends Obj, Props extends Record<string, Prop>>(
    ...
    const updateState = useCallback(() => {
        ...
        const keys = Object.keys(allProps[0]).filter((key) => allProps.every((prop) => key in prop)) as Array<
            keyof Props
        >;
        if (keys.length === 0) {
            setProps(undefined);
        ...
        const newProps = Object.fromEntries(
            keys.map((key) => [key, allProps.map((p) => p[key])]),
    ...
    useEffect(() => {
        const getSelections = () => (ignore ? circuit.selections.filter((s) => !ignore(s)) : circuit.selections.all);
