...
export const PropertyModule = ({ designer, propInfo }: Props) => {
    ...
    return (
        <>
            {infoList.map((entry) => (
                <PropInfoEntryWrapper key={entry.id} circuit={circuit} entry={entry} props={props} objs={objs} />
            ))}
        </>
    );
};
...
const PropInfoEntryInputField = ({ circuit, entry, props, objs }: PropInputFieldProps) => {
    ...
    switch (entry.type) {
                ...
                <div>
                    ...
                    <span style={{ display: "inline-block", width: "30%" }}>
                        <SelectModuleInputField
                            ...
                            doChange={(newUnits: string[]) =>
                                objs.forEach((o, i) => {
                                    o.setProp(unit.key, newUnits[i]);
                                    // Need to transform prop to new unit
                                    o.setProp(
                                        key,
                                        ((vals as number[])[i] * unit.entries[unitVals[i]].scale) /
                                            unit.entries[newUnits[i]].scale,
                                    );
                                })
                            }
                        />
        ...
        case "string":
            return <TextModuleInputField circuit={circuit} props={vals as string[]} doChange={doChange} />;
        case "string[]":
            return (
                <SelectModuleInputField
                    circuit={circuit}
        ...
        case "color":
            return <ColorModuleInputField circuit={circuit} props={vals as string[]} doChange={doChange} />;
    }
};
