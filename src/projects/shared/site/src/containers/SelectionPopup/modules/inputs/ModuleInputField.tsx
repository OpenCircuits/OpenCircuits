...
export const DefaultConfig = <V extends Primitive>({
    ...
    onSubmit,
    getCustomDisplayVal: getCustomDisplayVal ? ([v]) => getCustomDisplayVal!(v) : undefined,
});
...
export const useBaseModule = <V extends Primitive[]>({
    ...
    const values = indices.map((i) =>
        focused ? textVals[i] : allSame[i] ? (getCustomDisplayVal?.(val0, i) ?? val0[i]) : "",
    ...
    const onChange = (newVal: string, i = 0) => {
        ...
        setState(({ textVals, modifiers, setProps, ...prevState }) => {
            ...
            const moddedProps = newProps.map(
                (prop, j) =>
    ...
    const onBlur = () => {
        setState(({ modifiers, setProps, initialProps, submission }) => {
            ...
            if (finalProps.every((prop, j) => prop.every((val, i) => val === initialProps[j][i]))) {
                return initialState;
            }
