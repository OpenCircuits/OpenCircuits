...
export const SelectionPopup = ({ designer, docsUrlConfig, children }: Props) => {
    ...
    useEffect(() => {
        ...
        return circuit.selections.subscribe(() => {
            ...
            setClickThrough((prevState) => {
                if (prevState)
                    // If already have a timeout then just ignore
                    return prevState;
                setTimeout(() => setClickThrough(false), DOUBLE_CLICK_DURATION);
                return true;
            });
        });
    ...
    useEffect(
        () => viewport.camera.subscribe(() => setPos(viewport.toScreenPos(circuit.selections.midpoint))),
        [circuit, setPos],
    );
    ...
    useEvent(
        ...
        designer.viewport.canvasInfo?.input,
        [setIsDragging],
    );
    useEvent(
        "mouseup",
        ...
        designer.viewport.canvasInfo?.input,
        [circuit, viewport, setPos, setIsDragging],
    );
    ...
    const finalPos = (() => {
        ...
        if (!popup.current) throw new Error("SelectionPopup failed: popup.current is null");
