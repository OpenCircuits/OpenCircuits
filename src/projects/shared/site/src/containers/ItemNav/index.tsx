...
export const ItemNav = <D,>({
    ...
    const { isOpen, isEnabled, isHistoryBoxOpen, curPressedObjID } = useSharedSelector((state) => ({
        ...state.itemNav,
        curPressedObjID: state.circuit.curPressedObjID,
    }));
    ...
    function handleItemNavDrag() {
        // Issue #478
        if (!curPressedObjID) return;
        const curPressedObj = circuit.getObj(curPressedObjID)!;
        if (curPressedObj.baseKind !== "Component") return;
    ...
    useDocEvent(
        ...
        (ev) => {
            ...
            if (!touch) throw new Error("ItemNav.useDocEvent failed: touch is null");
    ...
    useEffect(() => {
        if (isShiftDown)
            // Don't reset on click if shift is down
            return;
    ...
    const deleteImg = useMemo(() => {
        ...
        if (!hoveringNav || !curPressedObjID) return;
        const obj = circuit.getObj(curPressedObjID)!;
        if (obj.baseKind !== "Component") return;
    ...
    return (
        <>
            ...
            <nav
                ...
                onFocus={() => setHoveringNav(true)}
                onMouseLeave={() => setHoveringNav(false)}
                onBlur={() => setHoveringNav(false)}
...
const ItemNavItemPreview = <D,>({
    curItemID,
    numClicks,
    curItemImg,
    additionalData,
    additionalPreview,
}: ItemNavItemPreviewProps<D>) => {
    const MAX_STACK = 4;
    ...
    return (
        ...
        >
            ...
            <span style={{ display: numClicks > MAX_STACK ? "initial" : "none" }}>x{numClicks}</span>
        </div>
    );
};
...
type ItemProps<D> = {
    ...
    readonly additionalData?: D;
    readonly setCurItemState: React.Dispatch<
...
const ItemNavButton = <D,>({
    ...
    return (
        <div
            ...
            tabIndex={0}
            onMouseEnter={() => {
                item.removable && setHovering(true);
            }}
            onMouseLeave={() => {
                item.removable && setHovering(false);
            }}
        >
            <Draggable
                dragDir={dragDir}
