...
const getOpInfo = (op: CircuitHistoryOp): OpInfo => {
    switch (op.kind) {
        ...
        case "ConnectWireOp":
            ...
            return {
                displayName: `${op.inverted ? "Removed Connection From" : "Connected"} ${op.w.p1} to ${op.w.p2}`,
                extraInfo: `ID: ${op.w.id}`,
            };
        case "CreateICOp":
            return {
                displayName: `${op.inverted ? "Removed" : "Created"} IC ${op.ic.metadata.name}`,
                extraInfo: `ID: ${op.ic.metadata.id}`,
            };
        ...
        case "SetComponentPortsOp":
            return {
                displayName: "Set Component Ports",
                extraInfo: `Added ${op.addedPorts.length} ports, removed ${op.removedPorts.length} ports`,
            };
        case "SetPropertyOp":
            return {
                displayName: `Changed Property ${op.key}`,
                extraInfo: `Changed property ${op.key} from ${op.oldVal} to ${op.newVal} on ${op.id}`,
            };
        case "SplitWireOp":
...
const HistoryEntry = ({ op, isRedo }: HistoryEntryProps) => {
    ...
    return (
        <div
            className={`historybox__entry ${isRedo ? "historybox__entry--dashed" : ""}`}
            role="button"
            tabIndex={0}
            onClick={(e) => {
        ...
        >
            <div className="historybox__entry__header">
                {opInfo.extraInfo && <img src={infoIcon} height="24px" alt="Display extra info" />}
                <span>{opInfo.displayName}</span>
            </div>
            {!displayExtraInfo && opInfo.extraInfo && (
                <div className="historybox__entry__extrainfo">{opInfo.extraInfo}</div>
            )}
        </div>
    );
};
...
const GroupActionEntry = ({ a, isRedo }: GroupActionEntryProps) => {
    ...
    return (
        <div
            className={`historybox__groupentry ${isRedo ? "historybox__groupentry--dashed" : ""}`}
            role="button"
            tabIndex={0}
            onClick={(e) => {
                // Necessary to stop child entries from collapsing the parent history entry
        ...
        >
            ...
            {!isCollapsed &&
                a.ops.map((op, i) => <HistoryEntry key={`group-action-entry-${i}`} op={op} isRedo={isRedo} />)}
...
export const HistoryBox = ({ designer }: Props) => {
    ...
    const { isOpen, isHistoryBoxOpen, curItemID } = useSharedSelector((state) => ({ ...state.itemNav }));
    ...
    return (
        ...
        >
            <div className={`historybox ${isOpen ? "" : "historybox__move"}`} data-adjustable>
                <div data-adjustable>
                    <span data-adjustable>History</span>
                    <span role="button" tabIndex={0} data-adjustable onClick={() => dispatch(CloseHistoryBox())}>
                        ×
                    </span>
                </div>
                <div data-adjustable>
