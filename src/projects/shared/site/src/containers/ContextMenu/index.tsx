...
function isClipboardSupported(type: "read" | "write"): boolean {
    return (
        ...
        (type === "read" ? navigator.clipboard.readText !== undefined : navigator.clipboard.writeText !== undefined)
    );
}
...
export const ContextMenu = ({ designer }: Props) => {
    ...
    const { isOpen, isLocked } = useSharedSelector((state) => ({
    ...
    useDocEvent(
        ...
        (ev) => {
            ...
            if (ev.target instanceof HTMLCanvasElement) {
                // Only open when clicking on a canvas
    ...
    const onCopy = async () => {
        ...
        await navigator.clipboard.writeText(CircuitHelpers.SerializeAsString(circuit.selections.withWiresAndPorts()));
    };
    ...
    return (
        ...
        >
            <button
                ...
                onClick={() => doFunc(onCut)}
            >
                Cut
            </button>
            <button type="button" title="Copy" disabled={circuit.selections.isEmpty} onClick={() => doFunc(onCopy)}>
                Copy
            </button>
            <button type="button" title="Paste" disabled={isLocked} onClick={() => doFunc(onPaste)}>
                Paste
            </button>
            <button
                type="button"
                ...
                onClick={() => doFunc(onSelectAll)}
            >
                Select All
            </button>
            <hr />
            <button type="button" title="Focus" onClick={() => doFunc(onFocus)}>
                Focus
            </button>
            <button
                type="button"
