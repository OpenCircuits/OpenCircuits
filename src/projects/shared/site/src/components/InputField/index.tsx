...
export const InputField = React.forwardRef<HTMLInputElement, Props>(({ onEnter, ...props }, forwardedRef) => {
    ...
    useEffect(() => {
        ...
        if (!cur) throw new Error("InputField.useEffect failed: ref.current is null");

        const onKeyUp = (evt: KeyboardEvent) => {
            if (evt.key === "Escape" || evt.key === "Enter") cur.blur();
...
export const NumberInputField = React.forwardRef<HTMLInputElement, NumberProps>(
    ({ onIncrement, step, ...props }, forwardedRef) => {
        const defaultRef = useRef<HTMLInputElement>(null);
        const ref = (forwardedRef ?? defaultRef) as React.RefObject<HTMLInputElement>;
                ...
                {!!step && (
                    <span>
                        <button
                            ...
                            onClick={() => onIncrementClick(+(step ?? 1))}
                        >
                            &and;
                        </button>
                        <button
