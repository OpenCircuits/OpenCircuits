...
export function MakeConversionMaps(
    ...
    function MakeAndCheckCompInfo(r: Record<string, CompConversionInfo>) {
        ...
        if (new Set(kindsMap.values()).size !== kindsMap.size) throw new Error("Duplicate keys found in KindMap!");
        const portsMap = new Map(
            [...Object.entries(r)].map(([kind, info]) => [kind, new Map(Object.entries(info.ports))]),
        );
    ...
    return [
        forwardMap,
        {
            ...
            portGroups: new Map(
                [...forwardMap.portGroups.entries()].map(([kind, ports]) => [kind, InvertMap(ports)] as const),
            ),
        },
    ];
}
...
export function CircuitToProto(
    circuit: Circuit,
    version: string,
    conversionInfo: ForwardConversionInfo,
): ProtoSchema.Circuit {
    ...
    function GetPortGroups(c: ReadonlyComponent, ics: IntegratedCircuit[]): Map<string, number> {
        if (!c.isIC()) return conversionInfo.portGroups.get(c.kind)!;
    ...
    function ConvertICMetadata(
        ic: IntegratedCircuit,
        components: ReadonlyComponent[],
    ): ProtoSchema.IntegratedCircuitMetadata {
...
export function ProtoToCircuit<C extends Circuit>(
    proto: ProtoSchema.Circuit,
    mainCircuit: C,
    CreateCircuit: (id: GUID) => C,
    conversionInfo: BackwardConversionInfo,
): C {
    ...
    function GetPortGroups(c: Component, ics: ProtoSchema.IntegratedCircuit[]): Map<number, string> {
        ...
        if (!ic) return conversionInfo.portGroups.get(c.kind)!;
        return InvertMap(new Map(Object.entries(ic.metadata!.portGroups!)));
    }

    function SetObjects(
        circuit: Circuit,
        objs: { components: ProtoSchema.Component[]; wires: ProtoSchema.Wire[] },
        ics: ProtoSchema.IntegratedCircuit[],
    ) {
        ...
        for (let i = 0; i < objs.components.length; i++) {
            ...
            if (c.portConfigIdx !== undefined) comp.setPortConfig(comp.info.portConfigs[c.portConfigIdx]);
    ...
    function SetIC(
        circuit: Circuit,
        ic: ProtoSchema.IntegratedCircuit,
        allICs: Map<GUID, ProtoSchema.IntegratedCircuit>,
        ics: ProtoSchema.IntegratedCircuit[],
    ) {
        const id = ic.metadata!.metadata!.id;

        // If IC already added, skip
        if (circuit.getIC(id)) return;

        // Find IC dependencies and load them first
        const dependencies = new Set(
            ic.components.map((c) => ConvertICIdx(c.icIdx, ics)).filter((id) => id !== undefined),
        );
        for (const depId of dependencies) SetIC(circuit, allICs.get(depId)!, allICs, ics);
    ...
    if (!proto.metadata) throw new Error(`ProtoToSchema: Failed to find metadata! ${proto}`);
    ...
    const icMap = new Map(proto.ics.map((ic) => [ic.metadata!.metadata!.id, ic]));
    for (const ic of proto.ics) SetIC(circuit, ic, icMap, proto.ics);
