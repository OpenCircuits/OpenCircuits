                        ...
                        onAuthStateChanged(auth, async (user) => {
                            if (user) {
                                await store.dispatch(
                                    Login(new GoogleAuthState(await user.getIdToken(), user.displayName ?? "")),
                                );
                            }
                        });
                    },
                        ...
                        const designer = CreateDesigner(
                            tools?.config ?? {
                                defaultTool: new DefaultTool(
                                    ...
                                    SaveHandler(
                                        () => store.getState().user.isLoggedIn /* && helpers.SaveCircuitRemote() */,
                                    ),
                                ),
                                tools: [
                                    new PanTool(),
                            ...
                            tools?.renderers ?? [
                                RotateToolRenderer,
                                WiringToolRenderer(({ renderer }) => renderer.options.defaultWireColor),
                                SelectionBoxToolRenderer,
                            ],
                            DRAG_TIME,
                            circuit,
                        );
