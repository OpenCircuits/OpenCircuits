...
export function MakeAnalogCircuitAssembler(circuit: CircuitInternal, options: RenderOptions): CircuitAssembler {
    return new CircuitAssembler(circuit, options, (params) => ({
        ...
        "Capacitor": new SVGComponentAssembler(
            params,
            { "": (_, i) => [LEFT, RIGHT][i] },
            V(0.4, 1.2),
            "capacitor.svg",
        ),
        "Inductor": new SVGComponentAssembler(
            params,
            { "": (_, i) => [TOP, BOTTOM][i] },
            V(0.8, (240 / 104) * 0.8),
            "inductor.svg",
