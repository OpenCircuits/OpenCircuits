...
export class SeparatedComponentCollection {
    ...
    public getEverything(): Array<EEObject> {
        return (<Array<EEObject>>this.getAllComponents()).concat(this.wires);
    }
 ...
 */
export function GetAllPorts(objects: Array<EEComponent> | SeparatedComponentCollection): Array<EEPort> {
...
export function CreateGraph(groups: SeparatedComponentCollection): Graph<number, number> {
    ...
    const map = new Map<EEComponent, number>();
...
export function CopyGroup(objects: Array<EEObject> | SeparatedComponentCollection): SeparatedComponentCollection {
    ...
    // Copy components
    const copies: Array<EEComponent> = [];
    for (const obj of objs)
        copies.push(obj.copy());


    // Copy connections
    const wireCopies: Array<EEWire> = [];
    ...
    const group: Array<EEObject> = copies;
...
export function SaveGroup(node: XMLNode, objects: Array<EEComponent>, wires: Array<EEWire>): void {
    ...
    const idMap = new Map<EEObject, number>();
...
export function LoadGroup(node: XMLNode): SeparatedComponentCollection {
    ...
    const idMap = new Map<number, EEComponent>();

    const objects: Array<EEObject> = [];
    const wires: Array<EEWire> = [];
