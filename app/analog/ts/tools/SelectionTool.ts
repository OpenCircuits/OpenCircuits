import {LEFT_MOUSE_BUTTON,
        ...
        ESC_KEY, A_KEY, X_KEY,
        IO_PORT_RADIUS} from "analog/utils/Constants";
...
export class SelectionTool extends Tool {
    ...
    private shouldDeselect(obj: EEObject, shift: boolean): boolean {
    ...
    public onClick(input: Input, button: number): boolean {
        ...
        if (this.interactionHelper.click())
            return true;
    ...
    public onKeyDown(input: Input, key: number): boolean {
        ...
        if (key == DELETE_KEY || key == BACKSPACE_KEY) {
            ...
            const objs = selections.filter(o => o instanceof EEObject) as Array<EEObject>;
        ...
        if (key == X_KEY) { // Snip wire port(s)
            const selections = Array.from(this.selections);
            const wirePorts = selections.filter((o) => o instanceof Node) as Array<Node>;
    ...
    public calculateMidpoint(): Vector {
        const selections = Array.from(this.selections);
        const positions = selections.filter((s) => s instanceof EEComponent)
                .map((s) => s as EEComponent)
                .map((c) => c.getPos());
